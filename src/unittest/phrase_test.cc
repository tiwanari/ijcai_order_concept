...
TEST_F(PhraseTest, createAPhraseUsingVector)
{
    ...
    const Morph kMorph0(kInputText0);
    const std::string kInputText1 =
        "ました	接尾辞,動詞性接尾辞,"
        "動詞性接尾辞ます型,タ形,ます,既知語";
    const Morph kMorph1(kInputText1);
    const std::vector<Morph> kMorphs = {kMorph0, kMorph1};
    Phrase phrase(kMorphs);

    ASSERT_EQ(kMorph0.morph() + kMorph1.morph(), phrase.phrase());
    ASSERT_EQ("切り返しました", phrase.phrase());

    const std::vector<Morph>& morphs = phrase.morphs();
