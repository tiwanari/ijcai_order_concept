...
output_path = "#{data_path}/svm"
tune_data_path = "#{output_path}/#{TUNE_PATH_SUFFIX}"
test_data_path = "#{output_path}/#{TEST_PATH_SUFFIX}"
svm_path = "../../../tools/linear_svm/liblinear-ranksvm-2.1" # assume this folder exists
...
Parallel.each(0...num_of_data, in_processes: 40) do |i|
    ...
    c_params.each do |c_value|
        ...
        # "Usage: $0 train_test_output_path svm_path [c_value (default: 0.01)]
        #                       [kernel (default: 0)] [ablation (default: 0 = do not)]"
        # "e.g. $0 ../../result/gather/ver1/svr ../../tools/svm_rank/src [0.01] [0] [0]"
        ...
        # "e.g. #{__FILE__} ../../result/gather/ver1 ../../result/gather/ver1/svm_result.txt 0.01"
        svm_rank_output = "#{tune_data_sub_path}/eval/svm_result_ep#{epsilon}_c#{c_value}_k#{kernel}.csv"
        FileUtils.mkdir_p(File.dirname(svm_rank_output))
        STDERR.puts "./concat_svm_results.rb #{tune_data_sub_path} #{svm_rank_output} #{epsilon}"
        puts_line("concate_svm_results", OUT_LENGTH, STDERR)
        `./concat_svm_results.rb #{tune_data_sub_path} #{svm_rank_output} #{epsilon}`
